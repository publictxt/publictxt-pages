...
and `docs/decisions/` hold the **reasoning**, so the two can diverge honestly — a decision can
...
| # | Decision |
|---|---|
| [D1](decisions/D1.md) | No PublicTxt.Syntax / .NET dependency |
| [D2](decisions/D2.md) | A sync step is unavoidable |
| [D3](decisions/D3.md) | Hashtag extraction is preprocessed, not native |
| [D4](decisions/D4.md) | Dates come from a ladder, not from build time |
| [D5](decisions/D5.md) | Standard Markdown links only; `[[wikilinks]]` unsupported in v1 |
| [D6](decisions/D6.md) | Browse lists are client-side over a JSON index |
| [D7](decisions/D7.md) | One site-wide page index, not one per list page |
| [D8](decisions/D8.md) | Custom Pagefind UI, not the stock one |
| [D9](decisions/D9.md) | Plain CSS, no Sass |
| [D10](decisions/D10.md) | Section order is configured, not derived |
...
|---|---|---|
| `[[wikilink]]` conversion at sync time | No repo in hand where it bites; the sync step already has the hook (see [D5](decisions/D5.md)) | Someone builds against a repo with legacy wikilinks |
...
| Tag co-occurrence / relatedness | Was not expressible when each list page only saw its own slice; [D7](decisions/D7.md) makes it possible for the first time | Anyone wanting "related pages" — the data is now there |
