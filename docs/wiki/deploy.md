...
[publish-to-github-pages.yml](../../deploy/publish-to-github-pages.yml) is a
**template for the content repo**, not a workflow that runs here. The content repo
stays pure Markdown; this repo is checked out beside it at build time, so upgrading
the site is a one-line `ref` change.
...
which makes every page report the same date at both ends. Sync warns — check the
build log if dates look wrong. See [features/dates.md](features/dates.md).
