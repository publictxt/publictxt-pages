...
[categories.md](categories.md)), `rating` (see [ratings.md](ratings.md)).
...
category single-select, tags AND-ed) with counts *within the current result set*, and year as a select beside Sort, plus a minimum-rating select, whole-number steps, with an *Unrated* option ([ratings.md](ratings.md)).
...
`?tag=a&tag=b&type=wiki&category=essay&year=2024&rating=4&sort=title&page=2` (`q` reserved for search), spelled
